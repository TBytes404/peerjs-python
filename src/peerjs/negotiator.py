...
class Negotiator:
        ...
        **options,
        ...
        if originator:
            ...
            if self.connection.type == ConnectionType.Data:
                ...
                log.info(
                    "creating datachannel with label=%s and ordered=%s",
        ...
        def peerconn_oniceconnectionstatechange():
            ...
            async def on_failed(error=None):
                ...
                self.connection.emit(
                    ...
                    ConnectionError(f"Negotiation of connection to {peerId} failed."),
                )
            ...
            def on_closed():
                log.info(
                    f"iceConnectionState is closed, closing connections to {peerId}"
                )
                self.connection.emit(
                    ConnectionEventType.Error,
            ...
            def on_completed():
                log.debug("iceConnectionState completed for peer id %s", peerId)
            ...
            log.debug("iceConnectionState event: %s", peerConnection.iceConnectionState)
            ...
            self.connection.emit(
                ConnectionEventType.IceStateChanged, peerConnection.iceConnectionState
            )
    ...
    async def cleanup(self) -> None:
        ...
        log.debug("Closing and cleaning up PeerConnection to %s", self.connection.peer)
        ...
        if self.connection.type == ConnectionType.Data:
            ...
            if dataChannel:
                dataChannelNotClosed = (
                    dataChannel.readyState and dataChannel.readyState != "closed"
                )
    ...
    async def _makeOffer(self):
        ...
        try:
            ...
            if self.connection.options.sdpTransform and callable(
                self.connection.options.sdpTransform
            ):
            ...
            try:
                ...
                log.info(f"Set localDescription:{offer} for: {self.connection.peer}")
                payload = {
                    "sdp": offer,
                ...
                if (
                    err != "OperationError: Failed to set local offer sdp: "
    ...
    async def _makeAnswer(self) -> None:
        ...
        try:
            ...
            sdpTransformFunction = self.connection.options.get("sdpTransform", None)
            if sdpTransformFunction and callable(sdpTransformFunction):
                answer.sdp = sdpTransformFunction(answer.sdp) or answer.sdp
    ...
    async def handleSDP(self, type: ServerMessageType = None, sdp: dict = None) -> None:
        """Handle an SDP."""
    ...
    async def handleCandidate(self, ice=None):
        ...
        provider = self.connection.provider
        try:
            log.debug("Adding ICE candidate for peer id %s", self.connection.peer)
    ...
    def _addTracksToConnection(
        ...
        if not peerConnection.addTrack:
            return log.error(
                f"Your browser does't support RTCPeerConnection#addTrack. Ignored."
            )
    ...
    def _addStreamToMediaConnection(
        ...
        log.debug(
            f"add stream {stream.id} to media connection {mediaConnection.connectionId}"
        )
